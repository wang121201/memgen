...
public:
  ...
  uint64_t diagnostic_set_id(uint64_t index_addr)const{return set_index(index_addr);}

  struct ProtectionLine {uint64_t tag;uint32_t valid,dirty,reserved;};
  std::vector<ProtectionLine> diagnostic_protection_state(uint64_t index_addr)const{
    std::vector<ProtectionLine> out;for(const auto&e:sets_data_.at(set_index(index_addr)))out.push_back({e.tag,e.valid_sectors,e.dirty_sectors,e.reserved_sectors});return out;
  }
  struct CensusLine {uint64_t tag;uint32_t valid,dirty;};
  std::vector<CensusLine> diagnostic_census_state(uint64_t index_addr)const{
    std::vector<CensusLine> out;for(const auto&e:sets_data_.at(set_index(index_addr)))out.push_back({e.tag,e.valid_sectors,e.dirty_sectors});return out;
  }
  template<class F>void diagnostic_visit_resident(F visit)const{
    for(uint64_t s=0;s<sets_data_.size();++s)for(const auto&e:sets_data_[s])visit(s,e.tag,e.valid_sectors,e.dirty_sectors);
  }
  struct PressureSelection {EvictedLine payload;uint64_t dirty=0,eligible=0,partial=0,max_age=0,tags=0,sectors=0,rank=0,selected_age=0;};
  PressureSelection diagnostic_select_pressure(uint64_t index_addr,uint64_t now,
      const std::unordered_map<uint64_t,uint64_t>&last_write,unsigned age,uint64_t excluded_tag=UINT64_MAX)const{
    PressureSelection out;const auto &dq=sets_data_.at(set_index(index_addr));out.tags=dq.size();uint64_t rank=0;
    for(auto it=dq.rbegin();it!=dq.rend();++it){
      ++rank;
      if(!it->dirty_sectors)continue;
      ++out.dirty;out.sectors+=__builtin_popcount(it->dirty_sectors);auto w=last_write.find(it->tag);
      if(w==last_write.end() || w->second>now)throw std::runtime_error("missing dirty pressure timestamp");
      const auto elapsed=now-w->second;out.max_age=std::max(out.max_age,elapsed);
      if(it->dirty_sectors & ~it->valid_sectors){++out.partial;continue;}
      if(elapsed<age || it->tag==excluded_tag)continue;
      ++out.eligible;if(!out.payload.dirty){capture_dirty_run(*it,0,it->dirty_sectors,32,out.payload);out.rank=rank;out.selected_age=elapsed;}
    }
    return out;
  }

  template<class Age>
  PressureSelection diagnostic_select_sector_age(uint64_t index_addr,unsigned threshold,Age age)const{
    PressureSelection out;const auto&dq=sets_data_.at(set_index(index_addr));out.tags=dq.size();uint64_t rank=0;
    for(auto it=dq.rbegin();it!=dq.rend();++it){
      ++rank;if(!it->dirty_sectors)continue;
      ++out.dirty;out.sectors+=__builtin_popcount(it->dirty_sectors);
      uint32_t mask=0;uint64_t selected_min=UINT64_MAX;
      for(unsigned i=0;i<4;++i)if(it->dirty_sectors&(1u<<i)){
        const auto elapsed=age(it->tag*128+32*i);out.max_age=std::max(out.max_age,elapsed);
        if(elapsed>=threshold){mask|=1u<<i;selected_min=std::min(selected_min,elapsed);}
      }
      // Retain the original whole-line completeness gate to isolate age only.
      if(it->dirty_sectors & ~it->valid_sectors){++out.partial;continue;}
      if(!mask)continue;
      ++out.eligible;if(!out.payload.dirty){capture_dirty_run(*it,0,mask,32,out.payload);out.rank=rank;out.selected_age=selected_min;}
    }
    return out;
  }
  EvictedLine diagnostic_clean_pressure_mask(uint64_t index_addr,uint64_t address,uint32_t mask){
    auto&dq=sets_data_.at(set_index(index_addr));
    for(auto&entry:dq)if(entry.tag==address/line_size_){
      if(!mask||(mask&~entry.dirty_sectors)||(mask&~entry.valid_sectors))throw std::runtime_error("invalid explicit dirty mask");
      EvictedLine out;capture_dirty_run(entry,0,mask,32,out);
      if(out.incomplete_dirty_sectors||out.missing_dirty_bytes)throw std::runtime_error("unknown explicit clean payload");
      for(unsigned i=0;i<4;++i)if(mask&(1u<<i))entry.dirty_since[i]=0;
      entry.dirty_sectors&=~mask;return out;
    }
    throw std::runtime_error("explicit pressure tag absent");
  }
  EvictedLine diagnostic_clean_pressure(uint64_t index_addr,uint64_t address,bool single_sector=false){
    auto &dq=sets_data_.at(set_index(index_addr));
    for(auto &entry:dq)if(entry.tag==address/line_size_){
      if(!entry.dirty_sectors || (entry.dirty_sectors & ~entry.valid_sectors))throw std::runtime_error("invalid pressure clean");
      const uint32_t mask=single_sector?(entry.dirty_sectors & (~entry.dirty_sectors+1)):entry.dirty_sectors;
      EvictedLine out;capture_dirty_run(entry,0,mask,32,out);
      if(out.incomplete_dirty_sectors || out.missing_dirty_bytes)throw std::runtime_error("unknown clean payload");
      for(unsigned s=0;s<line_size_/32;++s)if(mask&(1u<<s))entry.dirty_since[s]=0;
      entry.dirty_sectors &= ~mask;return out;
    }
    throw std::runtime_error("pressure tag absent");
  }
  bool diagnostic_same_read_state(const SectorLruCache &other)const{
    if(line_size_!=other.line_size_ || assoc_!=other.assoc_ || sets_!=other.sets_ || set_index_function_!=other.set_index_function_)return false;
    for(size_t s=0;s<sets_data_.size();++s){const auto&a=sets_data_[s],&b=other.sets_data_[s];if(a.size()!=b.size())return false;
      for(size_t i=0;i<a.size();++i)if(a[i].tag!=b[i].tag || a[i].valid_sectors!=b[i].valid_sectors || a[i].reserved_sectors!=b[i].reserved_sectors || a[i].known_bytes!=b[i].known_bytes || a[i].reserved_until!=b[i].reserved_until)return false;
    }return true;
  }
  bool diagnostic_read_clean_first = false; // Private candidate; baseline defaults false.
  CacheAccess access(uint64_t addr, unsigned access_size, unsigned sector_size,
    ...
    if (streaming_fill)
      ...
      dq.push_front(entry);
    if (dq.size() > assoc_)
      evict_one(dq, set, sector_size, now, out.evicted, diagnostic_read_clean_first && operation == CacheOperation::Read, tag);
    out.result = CacheResult::LineMiss;
    return out;
  }
...
private:
  ...
  void evict_one(std::deque<LineEntry> &dq, uint64_t set,
                 unsigned sector_size, uint64_t now, EvictedLine &evicted, bool prefer_clean, uint64_t incoming_tag) const {
    ...
    if (prefer_clean) {
      // Only existing clean lines compete; otherwise retain normal LRU fallback.
      // This deliberately does not turn an all-dirty set into read bypass.
      for (auto victim = dq.rbegin(); victim != dq.rend(); ++victim) {
        refresh(*victim, now);
        if (victim->tag != incoming_tag && victim->dirty_sectors == 0 && victim->reserved_sectors == 0) {
          evict_at(victim);
          return;
        }
      }
