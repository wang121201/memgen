...
struct mem_instn {
  ...
  void drop_auxiliary_vectors() {
    std::vector<int>().swap(distance);
    std::vector<int>().swap(distance_L2);
    std::vector<bool>().swap(miss);
  }
...
public:
  ...
  void read_mem_instns(bool dump_log, std::vector<std::pair<int, int>> *x,
                       unsigned kernel_id);
  void set_drop_mem_auxiliary_vectors(bool drop) {
    drop_mem_auxiliary_vectors = drop;
  }
...
private:
  ...
  bool m_valid = false;
  bool drop_mem_auxiliary_vectors = false;
};
