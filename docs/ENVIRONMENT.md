...
Two kinds of check must be separated, because they answer different questions
and cost different things:
...
| Raw memory traces | **not produced at all**, see below | not obtainable from this entry point |
...
### 7.1 No per-address trace is reachable through this entry point

The frozen engine entry
`release/source/tools/hbserve_profile_stream_cache_semantic_r17.cpp` sets
`options.output_format = "summary"` unconditionally, so the `memgen` mode emits
aggregate counters only. The full-inference address stream is generated in
memory and streamed straight into the cache filter; it is never materialized.
Every receipt says so: `raw_trace_files` is false, `raw_trace_persisted` is
false, and `materialized_raw_sass_bytes` stays 0.

The record-emitting machinery does exist further down
(`hyfiss_request_trace_generator_stream_r4_semantic.cc` carries
`output_format`, `footprint_format`, `emit_level` and the
`accelsim_records_written` / `footprint_records_written` counters), so a trace
mode is a code change rather than a missing capability. It sits outside the
frozen boundary: that translation unit is hashed by
`release/current-core-manifest.json`, so changing it invalidates the derived
core identity that `scripts/verify_archive.py` checks. Treat any trace-output
work as a separate, reviewed change, and remember the archive's own rule that
