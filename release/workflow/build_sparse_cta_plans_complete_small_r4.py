#!/usr/bin/env python3
"""Build full-inference CTA plans with exact small-grid structural coverage."""
...
def main() -> int:
    ...
    for label, raw_app in args.target:
        ...
        profile_plan_path = output_root / f"{label}.profile.tsv"
        ...
        total_grid = 0
        total_selected = 0
        with plan_path.open("x", encoding="utf-8", newline="\n") as stream, profile_plan_path.open(
            "x", encoding="utf-8", newline="\n"
        ) as profile_stream:
            stream.write("kernel_id\tcta_ids\n")
            profile_stream.write("kernel_id\tmode\ttrain_ctas\tholdout_ctas\n")
            for kernel in expected:
                ...
                plan = (
                    {
                        ...
                        "train": list(range(grid)),
                        "holdout": [],
                    }
                    if grid <= COMPLETE_GRID_LIMIT else
                    census.cta_plan(dims, item["kernel_name"])
                )
                ...
                profile_stream.write(
                    f"{kernel}\t{plan['mode']}\t"
                    f"{','.join(str(value) for value in plan['train'])}\t"
                    f"{','.join(str(value) for value in plan['holdout'])}\n"
                )
        ...
        receipt = {
            ...
            "complete_grid_limit": COMPLETE_GRID_LIMIT,
            ...
            "profile_plan": artifact(profile_plan_path),
            "claim_boundary": (
                "Exact complete-grid selection through 512 CTAs, otherwise train plus "
                "disjoint holdout; no address capture, "
        ...
        rows.append({
            ...
            "plan": str(plan_path),
            "profile_plan": str(profile_plan_path),
