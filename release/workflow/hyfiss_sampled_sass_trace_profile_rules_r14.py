...
def fit_x_floor_quotient_y_table_z(
    points: list[tuple[int, int]],
    grid_dims: tuple[int, int, int],
    label: str,
) -> dict[str, Any]:
    """Fit grouped x coordinates plus categorical y and affine z.

    Tensor-core kernels can group adjacent x CTAs while advancing linearly in
    split-K z.  The x divisor remains uniquely selected from training data;
    every y category and each nontrivial z axis must be independently anchored
    before disjoint CTA holdouts are evaluated by ``profile``.
    """
    require(
        grid_dims[1] > 1 or grid_dims[2] > 1,
        f"{label}: multidimensional x-floor rule requires y or z extent > 1",
    )
    coordinates = [(block_coordinates(block, grid_dims), address) for block, address in points]
    anchor_coordinates, anchor_address = coordinates[0]
    require(anchor_coordinates == (0, 0, 0), f"{label}: multidimensional x-floor rule requires CTA (0,0,0)")
    ...
    normalized = [
        (
            coordinate[0],
            address - y_offsets[coordinate[1]] - z_stride * coordinate[2],
        )
        for coordinate, address in coordinates
    ]
    x_rule = fit_x_floor_quotient(
        normalized,
        (grid_dims[0], 1, 1),
        label,
    )
    return {
        "kind": "coordinate_x_floor_quotient_y_table_z",
        "intercept": x_rule["intercept"],
        "cta_x_divisor": x_rule["cta_x_divisor"],
        "cta_x_quotient_stride": x_rule["cta_x_quotient_stride"],
        "cta_y_offsets": y_offsets,
        "cta_z_stride": z_stride,
        "scope": "multidimensional_unique_training_fit_with_disjoint_holdout",
    }
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                try:
                    return fit_x_floor_quotient_y_table_z(points, grid_dims, label)
                except ValueError as multidimensional_floor_error:
                    try:
                        ...
                        try:
                            ...
                            raise ValueError(
                                ...
                                f"multidimensional x-floor fallback failed: {multidimensional_floor_error}; "
...
def predict_bases(
    ...
    for rule in entry["address_rules"]:
        if rule.get("kind") == "coordinate_x_floor_quotient_y_table_z":
            require(grid_dims is not None, "multidimensional CTA x-floor rule requires grid dimensions")
