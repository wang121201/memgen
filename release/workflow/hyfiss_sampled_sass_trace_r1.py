...
def collect_ctas_memc(paths: list[Path], selected: set[int]) -> tuple[dict[int, list[dict[str, Any]]], dict[str, Any]]:
    found: dict[int, list[dict[str, Any]]] = {block: [] for block in selected}
    artifacts = []
    records = 0
    ...
    for path in paths:
        ...
        for record in memc_records(path):
            ...
            for group in record["groups"]:
                require(
                    group["global_mask"] == parse_hex(record["mask"])
                    and group["local_mask"] == 0
                    and group["shared_mask"] == 0
                    and group["unknown_mask"] == 0,
                    "sampled raw generation only supports all-global MEMCv3 records",
                )
                for field in ("global_mask", "local_mask", "shared_mask", "unknown_mask"):
                    del group[field]
            found[record["block"]].append(record)
    missing = sorted(block for block, values in found.items() if not values)
    require(not missing, f"selected CTAs absent from MEMCv3 source: {missing}")
    return found, {
        "format": "memc_v3",
...
def fit_coordinate_rule(
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"{affine_error}; categorical-y fallback failed: {table_error}") from table_error
...
def profile(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.input_format == "raw":
        ...
        sampled, source = collect_ctas_memc(memc_paths, set(train + holdout))
        profile_schema = (
    ...
    if len(structural_groups) > 1:
        ...
        profile_schema = MEMCV3_STRUCTURAL_CLASS_PROFILE_SCHEMA

    template: list[dict[str, Any]] = []
    structural_classes: list[dict[str, Any]] = []
    cta_class_by_id: list[str] = []
    if profile_schema == MEMCV3_STRUCTURAL_CLASS_PROFILE_SCHEMA:
    ...
    result = {
        ...
        "status": (
            ...
            if profile_schema == MEMCV3_STRUCTURAL_CLASS_PROFILE_SCHEMA else
            "PASS_COMPLETE_GRID_NO_CTA_EXTRAPOLATION"
            if complete_grid else
            "PASS_EXACT_COORDINATE_RULE_TRAIN_AND_DISJOINT_HOLDOUT"
            if profile_schema == MEMCV3_COORDINATE_PROFILE_SCHEMA else
        ...
        "model": {
            ...
            "memcv3_lowering": "strict all-global records only; SM/function/warp retained for admission but raw generation uses captured issue.config SM mapping",
            "structure_selection": (
...
def generate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if value.get("schema") == MEMCV3_STRUCTURAL_CLASS_PROFILE_SCHEMA:
        ...
        for item in raw_classes:
            ...
            require(isinstance(class_template, list) and class_template, f"structural class {class_id} has no template")
        ...
        for record in memc_records(source):
            ...
            for group in record["groups"]:
                require(
                    ...
                    and group["shared_mask"] == 0
                    and group["unknown_mask"] == 0,
                    "MEMCv3 semantic audit only supports all-global records",
                )
            lanes += semantic_accumulate(record, by_block)
            records += 1
    return {
