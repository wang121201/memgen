...
def main() -> int:
    ...
    profile3 = {
        "schema": {"name": "hbserve.hyfiss_sampled_sass_profile", "version": 7},
        "status": "PASS_DETERMINISTIC_STREAM_BRIDGE_SMOKE",
        "kernel": {"id": 3, "name": "smoke_ring_wrap", "grid_size": 64},
        "template": [
            entry(
                ordinal,
                "LDG.E.32",
                ordinal,
                {
                    "intercept": 0x100000 + ordinal * 0x10000,
                    "cta_x_stride": 128,
                    "cta_y_stride": 0,
                    "cta_z_stride": 0,
                },
...
-kernel_3_kernel_name smoke_ring_wrap
-kernel_3_llama_phase decode_step_1
-kernel_3_grid_size 64
-kernel_3_block_size 32
-kernel_3_grid_dim_x 64
-kernel_3_grid_dim_y 1
-kernel_3_grid_dim_z 1
"""
...
"""
    by_sm: dict[int, list[str]] = {sm: [] for sm in range(48)}
    for block in range(64):
        by_sm[block % 48].append(f"(3,{block},{300 + block * 100})")
    issue = issue_prefix + "".join(
        f"-trace_issued_sm_id_{sm} {' '.join(tokens)}\n"
        for sm, tokens in by_sm.items()
        if tokens
    )
    ...
    with pack_path.open("wb") as pack, index_path.open("w", encoding="utf-8") as stream:
        for kernel, profile in enumerate((profile1, profile2, profile3), 1):
    ...
    expected = {
        "generated_memory_instructions": 5126,
        "generated_lane_addresses": 163976,
        "sector_requests": 20504,
        "read_sector_requests": 20496,
        "write_sector_requests": 8,
        "compact_bytes": 246048,
        "compact_sequence_prefix_records": 24,
        "compact_sequence_prefix": [
