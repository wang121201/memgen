    ...
    try:
        with pack_path.open("xb") as pack_stream, index_path.open("xb") as index_stream:
            for position, row in enumerate(rows, 1):
                ...
                holdout_ctas = tool.parse_ids(row["holdout_ctas"]) if row["holdout_ctas"] else []
                promoted_ctas: list[int] = []
                if not complete_grid:
                    if len(holdout_ctas) < 2:
                        raise RuntimeError(f"kernel {kernel}: cannot promote while retaining a holdout")
                ...
                index_row = {
                    ...
                    "promoted_training_ctas": promoted_ctas,
