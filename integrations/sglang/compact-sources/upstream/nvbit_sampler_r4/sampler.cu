// TASK-PRIVATE GENERATED DYNAMIC SAMPLER; see derivation.json.
// Full launch metadata; selected CTA memory packets only, exclusively via pipe.
#include <algorithm>
#include <atomic>
#include <cerrno>
#include <climits>
...
namespace sgsample {
bool prepare(CUcontext,CUfunction,nvbit_api_cuda_t,const std::string&,unsigned,unsigned,unsigned,unsigned,unsigned,unsigned,unsigned,uint64_t,const std::string&,const std::string&,uint64_t,const std::vector<uint64_t>&);
void complete(CUresult*);void context_end();void finish();std::string receipt_fields();bool is_closed();bool flush_ledger_closed();uint64_t flush_count();
}

namespace sgobs {
using std::string;
const uint64_t DEFAULT_CAP = 1024ull << 20;
const uint64_t RESERVED_FINISH = 2ull << 20;
const uint64_t MAX_LAUNCHES = 262144, MAX_FUNCTIONS = 16384;
static thread_local unsigned internal_depth = 0;
static std::atomic<uint64_t> internal_callbacks{0}, internal_dispatches{0}, internal_dispatch_returns{0};
...
struct State {
  ...
  void initialize() {
    ...
    emit(lifecycle_file,prefix("observer_init")+",\"nvbit_version\":"+quote(NVBIT_VERSION)+",\"nvbit_cuda_header_version\":"+number(CUDA_VERSION)+",\"dynamic_instrumentation\":true,\"code_hash_kind\":\"sha256_nvbit_decoded_instruction_rows_v1\",\"module_binary_hash_available\":false,\"static_instruction_rows_emitted\":false,\"max_total_bytes\":"+number(cap)+"}\n");
  }
...
string launch_body(CUcontext ctx,nvbit_api_cuda_t cbid,void *params) {
  ...
  const bool sample=sgsample::prepare(ctx,f,cbid,attrs,gx,gy,gz,bx,by,bz,shared,ptr(stream),d.code_hash,d.name,d.id,ids);
  nvbit_enable_instrumented(ctx,f,sample);
...
void State::finish(const string &why) {
  ...
  bool pass=closed&&errors.empty()&&failed_launches==0&&unsupported==0&&graph_nodes==0&&unknown_attrs==0&&sgsample::flush_ledger_closed()&&sgsample::is_closed();
  ...
  string out="{\"schema\":\"sg_nvbit_observer_finish_v1\",\"status\":"+quote(pass?"PASS_SAMPLER_HOST_CLOSED_AWAITING_CONSUMER":"FAIL_OR_UNSUPPORTED_SAMPLED_HOST")+",\"reason\":"+quote(why)+",\"pid\":"+number(owner)+",\"start_ticks\":"+number(ticks)+",\"output\":"+quote(path)+",\"launch_before_count\":"+number(launches)+",\"launch_return_count\":"+number(returned)+",\"launch_error_count\":"+number(failed_launches)+",\"unsupported_dispatch_count\":"+number(unsupported)+",\"graph_node_callback_count\":"+number(graph_nodes)+",\"unknown_launch_attribute_count\":"+number(unknown_attrs)+",\"unbound_launch_count\":"+number(unbound_launches)+",\"context_count\":"+number(contexts.size())+",\"open_context_count\":"+number(open_contexts)+",\"epoch_begin_count\":"+number(epoch_begun)+",\"epoch_end_count\":"+number(epoch_ended)+",\"active_epoch\":"+number(active_epoch)+",\"function_count\":"+number(functions.size())+",\"observed_launch_thread_count\":"+number(launch_threads.size())+",\"observed_stream_count\":"+number(streams.size())+",\"marked_epoch_thread_count\":"+number(epoch_threads.size())+",\"marked_epoch_stream_count\":"+number(epoch_streams.size())+",\"internal_inspection_callback_count\":"+number(internal_callbacks.load())+",\"internal_inspection_dispatch_count\":"+number(internal_dispatches.load())+",\"internal_inspection_dispatch_return_count\":"+number(internal_dispatch_returns.load())+",\"metadata_bytes_before_finish\":"+number(total)+",\"max_total_bytes\":"+number(cap)+",\"api_counts\":"+apis+",\"errors\":"+es+",\"files\":"+inventory+",\"dynamic_instrumentation\":true,\"memory_addresses_captured\":true,\"actual_sm_placement_captured\":true,\"kernel_argument_values_captured\":false,\"module_binary_hash_available\":false,\"static_instruction_rows_emitted\":false,\"static_code_hash_is_decoded_sass_not_cubin\":true,\"device_synchronization_inserted\":true,\"gpu_memory_allocated_by_observer\":true,\"native_callback_completeness_independently_proven\":false,\"template_or_gtsim_admission\":false}\n";
  out.pop_back();out.pop_back();out+=sgsample::receipt_fields();out+=",\"allocation_API_calls\":"+number(allocation_calls)+",\"unparsed_allocation_API_calls\":"+number(unparsed_allocation_calls)+",\"complete_allocator_coverage\":false}\n";
...
void nvbit_at_ctx_term(CUcontext ctx) {
  ...
  try{auto it=s.contexts.find(ptr(ctx));need(it!=s.contexts.end()&&!it->second.closed,"unmatched context termination");sgsample::context_end();it->second.closed=true;s.emit(s.lifecycle_file,s.prefix("context_end")+",\"context_id\":"+number(it->second.id)+",\"context_handle_u64\":"+number(ptr(ctx))+"}\n");}catch(const std::exception&e){fatal(e.what());}
}
void nvbit_at_term() { using namespace sgobs;State&s=state();if(!s.enabled||getpid()!=s.owner)return;std::lock_guard<std::mutex> lock(s.lock);try{sgsample::finish();s.finish("nvbit_at_term");}catch(const std::exception&e){fatal(e.what());} }
...
void nvbit_at_cuda_event(CUcontext ctx,int is_exit,nvbit_api_cuda_t cbid,const char *name,void *params,CUresult *status) {
  ...
  if(internal_depth){++internal_callbacks;if(dispatch(api)){if(is_exit)++internal_dispatch_returns;else ++internal_dispatches;}return;}
